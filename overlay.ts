...
interface LiveChatOverlayApi {
  ...
  /**
   * オーバーレイ本体（#${OVERLAY_WRAPPER_ID}）のz-indexを外部から動的に指定する。
   * サイトによっては「通常時はヘッダーより奥・シアターモードや全画面表示中は
   * 動画プレイヤーより手前」というように、状態に応じて必要なz-indexが変わり
   * 単一の固定値では両立できない場合がある（Twitchが該当。sites/twitch.ts側で
   * 状態を判定し、状態が変わるたびに呼び出す想定）。
   * 数値でない・NaN・負数は無視する。
   */
...
(function (): void {
  ...
  function setZIndex(zIndex: number): void {
    ...
    overlayWrapperEl?.style.setProperty(
      "--live-chat-overlay-z-index",
      String(zIndex)
    );
  }
  ...
  function watchSettingsChanges(): void {
    chrome.storage.onChanged.addListener((changes, areaName) => {
      ...
      if (changes[STORAGE_KEY_OPACITY]) {
        ...
        applyOpacity(
          clampOpacity(typeof newValue === "number" ? newValue : DEFAULT_OPACITY)
        );
      }
