...
(function (): void {
  ...
  /** iframe側から送られてくるコメント追加メッセージの型 */
  interface AddCommentMessage {
    type: typeof ADD_COMMENT_MESSAGE_TYPE;
    commentId: string;
    author: string;
    text: string;
  }
  ...
  function initTopFrame(): void {
    ...
    let hiddenChatFrame: HTMLIFrameElement | null = null;
    /** 直近で隠しiframeに設定したURL（ネイティブiframeのURL変化検知用） */
    let lastChatUrl: string | null = null;
    /** 再生位置の定期送信に使う setInterval のID（未実行時は null） */
    let videoProgressIntervalId: ReturnType<typeof setInterval> | null = null;
     ...
     */
    function postSyncMessageToHiddenFrame(
      message: { "yt-player-video-progress": number } | { "yt-player-state-change": number }
    ): void {
    ...
    function stopVideoProgressSync(): void {
     ...
     */
    function startVideoProgressSync(video: HTMLVideoElement): void {
     ...
     */
    function attachVideoSyncListeners(video: HTMLVideoElement): () => void {
     ...
     */
    function syncHiddenChatFrame(): void {
      const nativeIframe = document.querySelector<HTMLIFrameElement>(
        NATIVE_CHAT_IFRAME_SELECTOR
      );
      if (!nativeIframe) {
        return;
      }
    ...
    const processedCommentIdOrder: string[] = [];

    function isDuplicateComment(commentId: string): boolean {
    ...
    window.addEventListener("message", (event: MessageEvent) => {
      ...
      const data = event.data as Partial<AddCommentMessage> | null | undefined;
  ...
  function extractComment(
    el: Element
  ): { id: string; author: string; text: string } | null {
   ...
   */
  function dispatchComment(commentId: string, author: string, text: string): void {
    if (!window.top) {
      return;
    }
    const message: AddCommentMessage = {
  ...
  function initChatFrame(): void {
     ...
     */
    function ensureItemsObserved(): void {
