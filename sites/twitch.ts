...
(function (): void {
  ...
  const CHAT_COLLAPSE_TOGGLE_SELECTOR =
   ...
   */
  function extractBodyText(bodyEl: Element): string {
    let text = "";
   ...
   */
  function extractComment(el: Element): { author: string; text: string } | null {
   ...
   * すべて集めて返す。
   */
  function collectMatchingElements(node: Element, selector: string): Element[] {
  ...
  function handleAddedNode(node: Node): void {
    ...
    const messageEls = new Set<Element>([
   ...
   */
  function isNativeChatCollapsed(): boolean {
   ...
   */
  function reinitializeCollapsedChat(): void {
  ...
  function checkChannelChange(): void {
    ...
    if (lastPathname !== null && pathname !== lastPathname) {
      ...
      if (
        window.LiveChatOverlay?.isEnabled?.() !== false &&
   ...
   */
  function syncZIndex(): void {
    if (typeof window.LiveChatOverlay?.setZIndex === "function") {
      window.LiveChatOverlay.setZIndex(
        isImmersiveMode() ? IMMERSIVE_Z_INDEX : NORMAL_Z_INDEX
      );
    }
  }
  ...
  let observedChatContainerEl: Element | null = null;
   ...
   */
  function ensureChatContainerObserved(): void {
    const containerEl =
